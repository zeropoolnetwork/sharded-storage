...
use crate::state::{AppState, Command, NodeId, Peer};

// TODO: Abstract basic network functionality into a separate crate.
//       There is a lot of shared code between the node and the validator.
...
pub async fn start_network(
    ...
    let local_key = load_or_generate_keypair(&format!("data/validator-keypair"))?;
    ...
    // TODO: Organize event loop context.
    let mut handle_event = |event| async {
        match event {
    ...
    loop {
        let res: Result<()> = tokio::select! {
            event = swarm.select_next_some() => handle_event(event).await,
                ...
                match command {
                    Some(Command::UploadCluster { id, shards }) => {
                        ...
                        for (id, shard) in shards.into_iter().enumerate() {
                            let peer = peers.get(&(id as u32)).ok_or_else(|| Error::msg("Peer not found"))?;
