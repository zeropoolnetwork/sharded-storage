        ...
        let res: Result<()> = (|| async {
            match event {
                SwarmEvent::ConnectionEstablished {
                    ...
                    // FIXME: Should work with the QUIC transport. Do we care about other transport?
                ...
                SwarmEvent::Behaviour(BehaviourEvent::RequestResponse(
                    ...
                    request_response::Message::Request {
                        ...
                        Req::Upload { index, data, .. } => {
                            let elements: Vec<_> = bincode::deserialize(&data)?;
                            state.storage.write(index, &elements).await?;
                        }
                        Req::NewNode { id, peer } => {
